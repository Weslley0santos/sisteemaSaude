  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
    ...
    <q-card class="p-3">
      ...
      <q-date
        ...
        @update:model-value="
          emit('update:dataSelecionada', $event)
        "
        minimal
      />
...
const tempoMedioEspera = computed(() => {
  ...
  const total = store.atendimentos.reduce(
    (acc, atendimento) =>
      acc + atendimento.tempoAtendimento.espera,
    0
  );

  return Math.round(
    total / store.atendimentos.length
  );
});
...
const tempoMedioConsulta = computed(() => {
  ...
  const total = store.atendimentos.reduce(
    (acc, atendimento) =>
      acc +
      atendimento.tempoAtendimento.consultando,
    0
  );
...
const tempoMedioTotal = computed(() => {
  ...
  const total = store.atendimentos.reduce(
    (acc, atendimento) =>
      acc + atendimento.tempoAtendimento.total,
    0
  );
